...
const DEFAULT_IMPORT_URL = 'https://www.pcudashboard.com/api/automation/pcu-pulse';
...
async function queuePulseExport() {
  const username = requiredEnv('PCU_PULSE_USERNAME');
  const password = requiredEnv('PCU_PULSE_PASSWORD');
  const exportEmail = requiredEnv('PCU_PULSE_EXPORT_EMAIL');
  ...
  const athleteIds = [...new Set(items
    ...
    .filter(Boolean))];
  if (!athleteIds.length) throw new Error('PULSE returned no subscribed PCU athletes.');

  const startDate = pcuDate(2);
  const endDate = pcuDate(0);
  ...
  console.log(`Queued PCU PULSE export for ${athleteIds.length} athletes (${startDate} through ${endDate}).`);
}
...
async function filesFromMessage(message, accessToken) {
  ...
  for (const url of urlsFromBodies(messageBodies(message.payload))) {
    ...
    files.push({ kind, name: filename ? decodeURIComponent(filename) : `pcu_pulse_${kind}.csv`, buffer });
  }
  return files;
}
...
async function waitForPulseCsvs(requestedAt) {
  ...
  while (Date.now() < deadline) {
    ...
    if (found.has('events') && found.has('workload')) {
      console.log('Downloaded both PCU PULSE CSV exports from Gmail.');
...
async function importCsvs(files) {
  ...
  const response = await fetch(process.env.PULSE_IMPORT_URL || DEFAULT_IMPORT_URL, {
    method: 'POST',
    headers: { authorization: `Bearer ${requiredEnv('PCU_PULSE_SYNC_TOKEN')}` },
    body: form,
  });
  ...
  console.log('PCU PULSE import complete:', summary);
}
