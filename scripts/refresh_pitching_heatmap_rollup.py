...
WITH base_non_pro AS (
  ...
  SELECT
    ...
    END AS batterside_norm,
    CASE
      WHEN REGEXP_REPLACE(LOWER(COALESCE(pd."TaggedPitchType", '')), '[^a-z0-9]', '', 'g') IN ('fastball','fourseam','fourseamfastball','sinker','oneseamfastball') THEN 'Fastballs'
    ...
    END AS pitch_group,
    CASE
      WHEN REGEXP_REPLACE(LOWER(COALESCE(pd."TaggedPitchType", '')), '[^a-z0-9]', '', 'g') IN ('fourseam','fourseamfastball') THEN 'Fastball'
      WHEN REGEXP_REPLACE(LOWER(COALESCE(pd."TaggedPitchType", '')), '[^a-z0-9]', '', 'g') = 'oneseamfastball' THEN 'Sinker'
      ELSE COALESCE(NULLIF(TRIM(pd."TaggedPitchType"), ''), 'Unknown')
    END AS pitch_type,
    CASE
