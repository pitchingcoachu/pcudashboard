...
async function main() {
  const path = '/Users/jaredgaynor/Documents/GitHub/pcudashboard/trackman files/20260703-SargentsStad-1_unverified.csv';
  const sourceFile = '20260703-SargentsStad-1_unverified.csv';
    ...
    for (const row of rows) {
      ...
      const fileIdRes = await client.query(
        `INSERT INTO pitch_event_files (school_code, source_file, file_checksum, file_mtime, row_count)
         ...
         RETURNING file_id`,
        ['PCU', sourceFile, fileHash, rows.length]
      ...
      await client.query(
        ...
        ['PCU', fileId, sessionDate, 'Live', sourceFile, pitchUid,
