...
const dir = '/Users/jaredgaynor/Documents/GitHub/pcu/data/practice/backfill_202605_time';
const files = (await fs.readdir(dir)).filter((f) => f.toLowerCase().endsWith('.csv')).map((f) => path.join(dir, f));
const pairs = [];
for (const f of files) {
  const txt = await fs.readFile(f, 'utf8');
  const rows = parseCsv(txt);
  for (const r of rows) {
    const pitchuid = String(r.PitchUID ?? r.pitchuid ?? '').trim();
...
try {
  ...
  for (let i = 0; i < entries.length; i += chunk) {
    ...
    const sql = `
      ...
      WHERE pe.school_code = 'PCU'
        AND pe.session_date BETWEEN DATE '2026-05-15' AND DATE '2026-05-25'
  ...
  const check = await client.query(`
    SELECT
      ...
      SUM(CASE WHEN pitchuid IS NOT NULL AND btrim(pitchuid) <> '' THEN 1 ELSE 0 END)::int AS rows_with_pitchuid
    FROM pitch_events
    WHERE school_code='PCU'
      AND session_date BETWEEN DATE '2026-05-15' AND DATE '2026-05-25'
  `);
  console.log(JSON.stringify({ files: files.length, csv_pitchuid_time_pairs: pairs.length, unique_pitchuids: entries.length, rows_updated: updated, verify: check.rows[0] }, null, 2));
} finally {
  client.release();
  await pool.end();
}
