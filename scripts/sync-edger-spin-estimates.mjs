...
async function fit(video, output, pitch) {
  const args = [
    'scripts/edger_spin_cv.py', video, '--output', output, '--fit',
    '--spin-rate-rpm', String(pitch.spin_rate), '--capture-fps', '1000',
  ];
  if (pitch.ivb !== null && pitch.hb !== null) {
    args.push('--observed-ivb', String(pitch.ivb), '--observed-hb', String(pitch.hb));
  }
...
if (!/^[A-Z0-9_]+$/.test(schoolCode)) throw new Error('Invalid school code.');
const videoTable = `public.video_map_${schoolCode.toLowerCase()}`;
const pitchIds = String(option('--pitch-ids', '')).split(',').map((value) => Number(value.trim())).filter(Number.isInteger);
...
try {
  ...
  const result = await pool.query(`
    WITH pitches AS (
      SELECT DISTINCT ON (NULLIF(TRIM(pe.playid), ''))
        ...
        (regexp_match(COALESCE(pe.inducedvertbreak::text, ''), '[-+]?[0-9]*\\.?[0-9]+'))[1]::double precision AS ivb,
        (regexp_match(COALESCE(pe.horzbreak::text, ''), '[-+]?[0-9]*\\.?[0-9]+'))[1]::double precision AS hb,
        vm.cloudinary_url
      FROM public.pitch_events pe
      JOIN ${videoTable} vm
        ON NULLIF(TRIM(vm.play_id), '') = NULLIF(TRIM(pe.playid), '')
      ...
      WHERE pe.school_code = $3
        AND NULLIF(TRIM(pe.playid), '') IS NOT NULL
        AND NULLIF(TRIM(vm.cloudinary_url), '') IS NOT NULL
        AND lower(COALESCE(vm.video_type, '')) LIKE '%edger%'
        AND COALESCE(vm.camera_name, '') <> 'iPhone'
        AND ($4::bigint[] = '{}'::bigint[] OR pe.id = ANY($4::bigint[]))
  ...
  `, [MODEL_VERSION, limit, schoolCode, pitchIds]);
  ...
  for (const pitch of result.rows) {
    ...
    try {
      ...
      const output = join(work, 'fit-video-axis');
      const report = await fit(video, output, pitch);
      if (!report.camera_fit) throw new Error('no seam fit was produced');
      await run('node', [
        'scripts/import-edger-spin-estimate.mjs', '--pitch-event-id', String(pitch.id),
        '--report', join(output, 'report.json'), '--source-url', pitch.cloudinary_url,
      ], { env: process.env, maxBuffer: 2 * 1024 * 1024 });
      accepted += 1;
      await recordAttempt(pitch.id, 'accepted');
      console.log(`Accepted ${schoolCode} Edger seam fit for pitch_events.id=${pitch.id}`);
    } catch (error) {
      rejected += 1;
