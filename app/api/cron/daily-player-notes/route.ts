...
// matching DAILY_PLAYER_NOTES_CRON_KEY. Without also accepting CRON_SECRET,
// every real Vercel Cron invocation 401s and the route silently never runs.
function isAuthorized(request: Request): boolean {
  const configuredKey = String(process.env.DAILY_PLAYER_NOTES_CRON_KEY ?? '').trim();
