...
import { listNutritionLogsForPlayer, logSavedMeal } from '../../../../../../lib/training-db';
...
export async function POST(request: Request) {
  ...
  const savedMealId = Number(body.savedMealId ?? 0);
  ...
  const result = await logSavedMeal({
  ...
  return NextResponse.json({ ok: true, logIds: result.logIds, logs });
}
