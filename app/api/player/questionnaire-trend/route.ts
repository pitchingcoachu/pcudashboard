...
import { getPlayerByIdInOrganization, listQuestionnaireQuestionSeries } from '../../../../lib/training-db';

async function requireStaffSession(request: Request) {
  const cookieStore = await cookies();
  const session = getSessionFromRequest(request, cookieStore);
  if (!session) return { ok: false as const, status: 401, error: 'Unauthorized' };
  if (session.role === 'player') return { ok: false as const, status: 403, error: 'Only coaches and admins can view questionnaire trends.' };
...
export async function GET(request: Request) {
  ...
  if (!Number.isFinite(questionnaireId) || questionnaireId <= 0) return NextResponse.json({ error: 'Valid questionnaireId is required.' }, { status: 400 });
  ...
  const series = await listQuestionnaireQuestionSeries({ organizationId: scope.organizationId, playerId, questionnaireId, questionId });
