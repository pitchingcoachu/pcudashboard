...
import { getPlayerByIdInOrganization, listPlayerAnsweredQuestionnaireCatalog } from '../../../../lib/training-db';

async function requireStaffSession(request: Request) {
  const cookieStore = await cookies();
  const session = getSessionFromRequest(request, cookieStore);
  if (!session) return { ok: false as const, status: 401, error: 'Unauthorized' };
  if (session.role === 'player') return { ok: false as const, status: 403, error: 'Only coaches and admins can view questionnaire history.' };
...
export async function GET(request: Request) {
  ...
  const questionnaires = await listPlayerAnsweredQuestionnaireCatalog({ organizationId: scope.organizationId, playerId });
