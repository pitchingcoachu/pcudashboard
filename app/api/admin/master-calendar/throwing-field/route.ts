...
import { updateThrowingDayField } from '../../../../../lib/training-db';
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as
    | { playerId?: number; dayDate?: string; fieldKey?: string; value?: string }
    | null;
  ...
  const result = await updateThrowingDayField({
    organizationId,
    playerId,
    dayDate,
    fieldKey,
