...
import { getThrowingFieldSchema, saveThrowingFieldSchema } from '../../../../../lib/training-db';
...
export async function GET(request: Request) {
  ...
  const fields = await getThrowingFieldSchema({ organizationId });
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as { fields?: { key?: string; label?: string }[] } | null;
  ...
  const fields = Array.isArray(body.fields)
    ? body.fields.map((field) => ({ key: field?.key, label: String(field?.label ?? '') }))
    : [];

  const result = await saveThrowingFieldSchema({ organizationId, fields, userId });
