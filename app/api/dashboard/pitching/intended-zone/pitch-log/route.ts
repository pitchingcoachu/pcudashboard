...
import { listIntendedZonePitchLog } from '../../../../../../lib/training-db';
...
export async function GET(request: Request) {
  try {
    ...
    const pitches = await listIntendedZonePitchLog({
      organizationId,
    ...
    console.error('[intended-zone pitch-log] Unable to load pitch history.', error);
    return NextResponse.json(
      { error: 'Unable to load intended target pitch history. Please try again.' },
      { status: 500 },
    );
  }
}
