...
function isAuthorized(request: Request): boolean {
  const expected = String(process.env.PCU_PULSE_SYNC_TOKEN ?? '').trim();
  ...
  try {
    ...
    if (!kinds.has('events') || !kinds.has('workload')) {
      return NextResponse.json(
        { error: 'PCU automation requires one Events CSV and one Workloads CSV.' },
        { status: 400 },
      );
    }
    ...
    const message = error instanceof Error ? error.message : 'Unable to import PCU PULSE CSV files.';
