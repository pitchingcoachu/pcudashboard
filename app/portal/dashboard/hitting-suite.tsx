...
type ChartPoint = {
  ...
  bb_type_raw?: string | null;
  events_raw?: string | null;
...
function LocationChart({
  ...
  const rvMin = displayView === 'PV/100' ? -2 : -5;
  const rvMax = displayView === 'PV/100' ? 2 : 5;
  ...
  return (
    <div className="dashboard-panel" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <svg viewBox={`0 0 ${w} ${h}`} style={{ width: '100%', height: 340, border: '1px solid rgba(255,255,255,0.14)', borderRadius: 10 }}>
        ...
        {displayView !== 'Pitch' ? (
          <>
            <g filter={isRvLikeMetric ? `url(#hitting-loc-heat-blur-rv-${idBase})` : `url(#hitting-loc-heat-blur-${idBase})`}>
              {cells.map((cell) => {
                ...
                if (displayView === 'Frequency') fill = sequentialColor(cell.value, minVal, maxVal);
                  ...
                  fill = divergingColor(displayView === 'PV/100' ? rvClamped : -rvClamped, rvMin, 0, rvMax);
                }
                ...
                return (
                  <circle
                    ...
                    r={radius}
                    fill={fill}
                    opacity={Math.max(0.3, runValueBoost * 1.25 * (displayView === 'Frequency' ? 1 : Math.max(0.55, densityNorm)))}
                  />
                );
              })}
            </g>
            {cells.map((cell) => {
              ...
              const cx = xScale(cell.x + cell.w / 2);
              const cy = yScale(cell.y + cell.h / 2);
              const radius = Math.max(0.38, cell.w * ((w - pad * 2) / (xMax - xMin)) * 0.32);
              const densityNorm = Math.max(0, Math.min(1, cell.density / densityMax));
              let fill = 'rgba(255,255,255,0.12)';
              if (displayView === 'Frequency') fill = sequentialColor(cell.value, minVal, maxVal);
              else if (isRvLikeMetric) {
                ...
                <circle
                  ...
                  onMouseMove={(event) =>
                    onPointHover({
                      x: event.clientX,
                      y: event.clientY,
                      text: `${displayView}: ${cell.value.toFixed(displayView === 'xWOBA' || displayView === 'xISO' ? 3 : (isRvLikeMetric || displayView === 'Exit Velocity' ? 2 : 1))}`,
                    })
                  }
        ...
        <rect x={xScale(zoneLeft)} y={yScale(zoneTop)} width={xScale(zoneRight) - xScale(zoneLeft)} height={yScale(zoneBottom) - yScale(zoneTop)} fill="none" stroke="rgba(255,255,255,0.95)" strokeWidth="1.2" />
        <line x1={xScale(zoneLeft + (zoneRight - zoneLeft) / 3)} y1={yScale(zoneBottom)} x2={xScale(zoneLeft + (zoneRight - zoneLeft) / 3)} y2={yScale(zoneTop)} stroke="rgba(255,255,255,0.45)" strokeWidth="0.8" />
        <line x1={xScale(zoneLeft + (zoneRight - zoneLeft) * 2 / 3)} y1={yScale(zoneBottom)} x2={xScale(zoneLeft + (zoneRight - zoneLeft) * 2 / 3)} y2={yScale(zoneTop)} stroke="rgba(255,255,255,0.45)" strokeWidth="0.8" />
        <line x1={xScale(zoneLeft)} y1={yScale(zoneBottom - (zoneBottom - zoneTop) / 3)} x2={xScale(zoneRight)} y2={yScale(zoneBottom - (zoneBottom - zoneTop) / 3)} stroke="rgba(255,255,255,0.45)" strokeWidth="0.8" />
        <line x1={xScale(zoneLeft)} y1={yScale(zoneBottom - (zoneBottom - zoneTop) * 2 / 3)} x2={xScale(zoneRight)} y2={yScale(zoneBottom - (zoneBottom - zoneTop) * 2 / 3)} stroke="rgba(255,255,255,0.45)" strokeWidth="0.8" />
        <polygon
        ...
        {displayView === 'Pitch'
          ? plottedInZone.map((point, idx) => {
              ...
              const x = xScale(strictRunValue ? -ps : ps);
              const y = yScale(ph);
...
function buildHeatCells(points: ChartPoint[], metric: string, strictRunValue = false): HeatCell[] {
  if (metric === 'xWOBA' || metric === 'xISO') {
    const orientedPoints = points.map((point) => {
      ...
      const adjustedX = typeof rawX === 'number' ? (strictRunValue ? -rawX : rawX) : rawX;
  ...
  const normDesc = (value: string): string =>
    String(value ?? '')
      ...
      .replace(/[^a-z0-9]+/g, '_')
      .replace(/^_+|_+$/g, '');
  const isInPlayCall = (call: string): boolean => {
    if (!strictRunValue) return call === 'InPlay';
    const d = normDesc(call);
  ...
  const isWhiff = (call: string): boolean => {
    if (!strictRunValue) return call === 'StrikeSwinging';
    const d = normDesc(call);
    return d === 'strikeswinging' || d === 'swinging_strike' || d === 'swinging_strike_blocked' || d === 'foul_tip';
  };
  const isContact = (call: string): boolean => isInPlayCall(call);
  const isGroundBall = (point: ChartPoint) => {
    const norm = (value: unknown) =>
      String(value ?? '')
        ...
        .replace(/^_+|_+$/g, '');
    if (strictRunValue) {
      const bbType = norm(point.bb_type_raw);
      return bbType.includes('ground_ball') || bbType === 'groundball';
    }
    const tagged = norm(point.tagged_hit_type);
  ...
  const pvRows = valid.map((row) => pitchValue(row.p)).filter((value) => Number.isFinite(value));
...
export default function HittingSuite({
  ...
  const summaryPercentileDistributionCacheRef = useRef(
    new Map<string, { at: number; base: Map<string, number[]> }>()
  );
  ...
  useEffect(() => {
    ...
    overviewPromise
      .then(async (payload) => {
        ...
        const shouldTrySummaryHitterFallback =
          isPro &&
          isSummaryPage &&
          Boolean(summarySelectedHitter) &&
          (!Array.isArray(payload.table_rows) || payload.table_rows.length === 0);
        if (shouldTrySummaryHitterFallback) {
          ...
          fallbackParams.set('hitter', summarySelectedHitter);
          const dropKeys = [
            'opp_pitcher',
            'hand',
            'batter_side',
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    const run = async () => {
      const requests: Array<{ rowKey: string; query: string }> = [];
      for (const [rowKey, rowLabel] of rowsByKey.entries()) {
        ...
        if (!applyHittingSummarySplitFilter(params, splitBy, rowLabel)) continue;
        requests.push({ rowKey, query: params.toString() });
      }
      if (!requests.length) {
        if (!active) return;
        setSummaryPitchTypeDistributions(new Map());
      ...
      const allCandidateColumns = Array.from(
        new Set(
          [
            ...(Array.isArray(overview?.available_table_columns) ? overview.available_table_columns : []),
      ...
      const cols = allCandidateColumns.length ? allCandidateColumns : tableColumns.slice(1);
      if (controller.signal.aborted || !active) return;
      setLoadingSummaryPitchTypePercentiles(true);
      const response = await fetch('/api/dashboard/hitting/summary-percentiles', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        cache: 'no-store',
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const parseGameSplit = (raw: unknown): { date: string; team: string; opponent: string; gameKey: string; batterMarker: string } => {
        const token = String(raw ?? '').trim();
        const parts = token.split('||');
        if (parts.length >= 4) {
          const datePart = String(parts[0] ?? '').trim();
          const pitcherTeam = String(parts[1] ?? '').trim();
          const batterTeam = String(parts[2] ?? '').trim();
          const gameKey = String(parts[3] ?? '').trim() || token;
          return {
            date: datePart,
            team: batterTeam || '-',
            opponent: pitcherTeam || '-',
            gameKey,
            batterMarker: String(parts[5] ?? '').trim(),
          };
        }
    ...
    run()
      ...
      .finally(() => {
        if (!cancelled) setLoadingGameLog(false);
      });
    return () => {
      cancelled = true;
      controller.abort();
    ...
    filters?.opp_pitchers_by_team_code,
    isPro,
    level,
    oppPitcher,
    hand,
  ...
  const percentileDistributionsByKey = useMemo(() => {
    ...
    [scoped, globalByColumn].forEach((map) => {
  ...
  const getCellPercentile = useCallback((
    ...
    const { splitColumn, scoped, globalByColumn } = percentileDistributionsByKey;
    ...
    const distribution = isSummaryPageLocal
      ? (
          (() => {
            ...
            if (rowDistribution.length) return rowDistribution;
            if (isAllSummaryRow) return globalByColumn.get(column) ?? [];
  ...
  const leaderboardColumnRanges = useMemo(() => {
    ...
    const sourceRows = leaderboardRowsWithPins.filter((row) => {
      const firstCol = displayedTableColumns[0] ?? '';
      const label = String(row[firstCol] ?? '').trim().toLowerCase();
      return label !== 'all' && label !== 'all (pinned)';
    });
    for (const column of displayedTableColumns.slice(1)) {
      const values = sourceRows
        .map((row) => parseSortableNumber(row[column]))
    ...
    return ranges;
  }, [isLeaderboardPage, leaderboardRowsWithPins, displayedTableColumns]);
  ...
  const gameLogHeader = useMemo(() => {
    const primaryLabel = selectedSingleHitter
      ? formatNameFirstLast(selectedSingleHitter)
  ...
  }, [selectedSingleHitter, teamType, isPro, level, leagueTeamLabelByCode, latestTeamByHitter, filterTeamByHitter]);
  ...
  const heatmapsPageSvg = useMemo(() => {
    ...
    const cells =
      heatmapDisplayView === 'Pitch'
        ? []
        : buildHeatCells(heatmapPoints, heatMetricView, isPro);
    ...
    const fixedScale = getHeatmapFixedScale(heatMetricView, selectedHeatPitchTypes);
    const contactVisibilityScale = heatMetricView === 'Contact Rate' ? getHeatmapFixedScale('Whiff Rate', selectedHeatPitchTypes) : null;
    const minVal = fixedScale?.min ?? dynamicMinVal;
    const maxVal = fixedScale?.max ?? dynamicMaxVal;
    const midVal = fixedScale?.mid ?? dynamicMidVal;
    ...
    return (
      <svg viewBox={`0 0 ${w} ${h}`} style={{ width: '100%', height: 460, border: '1px solid rgba(255,255,255,0.16)', borderRadius: 10 }} onMouseLeave={() => setChartHover(null)}>
        ...
        <g transform={zoomTransform} clipPath="url(#hitting-heatmap-zoom-clip)">
          {heatmapDisplayView !== 'Pitch' ? (
            <>
              <g filter={isRunValuesMetric ? 'url(#hitting-heatmap-blur-rv)' : 'url(#hitting-heatmap-blur)'}>
                {cells.map((c) => {
              ...
              {cells.map((c) => {
                ...
                return (
                  <circle
                    ...
                    r={radius}
                    fill="rgba(0,0,0,0.001)"
                    onMouseMove={(event) => setChartHover({ x: event.clientX, y: event.clientY, text: `${heatmapDisplayView}: ${c.value.toFixed(heatmapDisplayView === 'xWOBA' || heatmapDisplayView === 'xISO' ? 3 : (isRunValuesMetric || heatmapDisplayView === 'Exit Velocity' ? 2 : 1))}` })}
          ...
          {heatmapDisplayView === 'Pitch'
            ? points
                ...
                .map((p, i) => {
                  const x = px(isPro ? -Number(p.plate_side) : Number(p.plate_side));
              ...
              <div className="dashboard-panel" style={{ padding: 14 }}>
                ...
                {!canRunGameLog ? (
                  <p className="portal-muted-text" style={{ marginBottom: 0 }}>
                    Game Log requires a selected team or player. Choose a team (not `All`) or filter to one or more hitters.
                  </p>
                ) : loadingGameLog ? (
                  <p className="portal-muted-text" style={{ marginBottom: 0 }}>Loading game log...</p>
                ) : gameLogError ? (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                      ...
                      <tbody>
                        {gameLogRowsWithPins.map((row, rowIndex) => (
                          <tr
                            key={String(row._game_pin_key ?? rowIndex)}
                            style={(() => {
                          ...
                          >
                            {(() => {
                              const rowKind = String(row._game_row_kind ?? '');
                              const isSummaryRow = rowKind === 'all' || rowKind === 'all_pinned';
                              const pinKey = String(row._game_pin_key ?? '');
                              const isPinnedRow = pinnedGameLogKeys.has(pinKey);
                              return (
                                <td style={{ textAlign: 'center' }}>
                                  {isSummaryRow ? '' : (
                                    <button
                                      type="button"
                                      aria-label={isPinnedRow ? 'Unpin game' : 'Pin game'}
                                      title={isPinnedRow ? 'Unpin' : 'Pin'}
                                      onClick={(event) => {
                                        event.preventDefault();
                                        event.stopPropagation();
                                ...
                                >
                                  {column === 'Team' && isPro && !isSummaryRow ? (
                                    (() => {
                                      ...
                                      return logo ? <img src={logo} alt={code || 'Team'} style={{ width: 18, height: 18, objectFit: 'contain' }} /> : <span>{cellValue === null || cellValue === undefined ? '—' : cellValue}</span>;
                                    })()
                                    ...
                                    (() => {
                                      ...
                                      return (
                                        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                                          ...
                                          {logo ? <img src={logo} alt={code || 'Opponent'} style={{ width: 18, height: 18, objectFit: 'contain' }} /> : <span>{cellValue === null || cellValue === undefined ? '—' : cellValue}</span>}
                                        </span>
                                      );
                                    })()
                                  ) : column === 'Team' && !isPro && !isSummaryRow ? (
                                    ...
                                    (() => {
                                      ...
                                      const markerText = markerRaw === '@' || markerRaw === 'away'
                                        ...
                                        : markerRaw === 'vs.' || markerRaw === 'vs' || markerRaw === 'home'
                                          ? 'vs. '
                                          : '';
                                      const valueText = mappedTeamLabel(rawValue);
                                      return `${markerText}${valueText}`;
                                    })()
                                  ) : (
                                    <span
                                      style={(() => {
