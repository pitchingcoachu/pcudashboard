...
import { canonicalFlagMetric, dashboardMetricLabel, dashboardMetricOptions, parseForcePlateFlagMetric } from '../../../lib/dashboard-metric-catalog';
import { formatTableDisplayValue } from '../../../lib/table-sort';

type Rule={id:number;name:string;domain:'pitching'|'hitting'|'force_plates';metric:string;pitchType?:string;pitchTypes:string[];direction:'increase'|'decrease'|'either';threshold:number;thresholdType:'absolute'|'percent';baselineDays:number;minimumSample:number;targetPlayer:string;sessionType:string;testType:string;notificationsEnabled:boolean;cooldownHours:number;enabled:boolean};
...
type MetricOptions=Record<'pitching'|'hitting'|'force_plates',string[]>;
type MetricSort={ruleId:number;direction:'asc'|'desc'}|null;
const EMPTY:Omit<Rule,'id'>={name:'',domain:'pitching',metric:'Velo',pitchTypes:['All'],direction:'increase',threshold:2,thresholdType:'absolute',baselineDays:30,minimumSample:5,targetPlayer:'All',sessionType:'All',testType:'All',notificationsEnabled:false,cooldownHours:24,enabled:true};
...
const domainLabel=(value:Rule['domain'])=>value==='force_plates'?'Force Plates':value==='hitting'?'Hitting':'Pitching';
const ruleFilterLabel=(rule:Rule)=>rule.domain==='force_plates'?`Test: ${rule.testType||'All'}`:`Pitch: ${(rule.pitchTypes?.length?rule.pitchTypes:[rule.pitchType??'All']).join(', ')}`;
...
const formatMetricValue=(value:number,metric:string)=>parseForcePlateFlagMetric(metric)?value.toFixed(1):formatTableDisplayValue(canonicalFlagMetric(metric),value);
...
export default function FlagsSuite(){
  const [view,setView]=useState<'active'|'rules'>('active'),[rules,setRules]=useState<Rule[]>([]),[results,setResults]=useState<Result[]>([]),[filterOptions,setFilterOptions]=useState<Record<'pitching'|'hitting'|'force_plates',FilterOptions>>({pitching:EMPTY_OPTIONS,hitting:EMPTY_OPTIONS,force_plates:EMPTY_OPTIONS}),[metricOptions,setMetricOptions]=useState<MetricOptions>({pitching:dashboardMetricOptions('pitching'),hitting:dashboardMetricOptions('hitting'),force_plates:[]}),[forcePlateTestTypes,setForcePlateTestTypes]=useState<string[]>([]),[forcePlateTestsByMetric,setForcePlateTestsByMetric]=useState<Record<string,string[]>>({}),[draft,setDraft]=useState<Omit<Rule,'id'>>(EMPTY),[editing,setEditing]=useState<number|null>(null),[metricSort,setMetricSort]=useState<MetricSort>(null),[loading,setLoading]=useState(true),[loadingOptions,setLoadingOptions]=useState(true),[message,setMessage]=useState('');
