...
// 'bullpen' reads/writes the pre-existing /api/player/bullpen-log route and its
// bullpenDate wire field; 'hitting' targets the mirrored /api/player/hitting-log
// route and hittingDate field. Both persist identically shaped rowsJson.
type ScriptEntryMode = 'bullpen' | 'hitting';

const MODE_CONFIG: Record<ScriptEntryMode, { logApiPath: string; dateField: 'bullpenDate' | 'hittingDate'; dateLabel: string; savedEntriesLabel: string; saveButtonLabel: string; emptyMessage: string }> = {
  bullpen: {
    logApiPath: '/api/player/bullpen-log',
    dateField: 'bullpenDate',
    dateLabel: 'Bullpen Date',
    savedEntriesLabel: 'Saved Bullpen Script Entries',
    saveButtonLabel: 'Save Bullpen',
    emptyMessage: 'No bullpen scripts assigned yet.',
  },
  hitting: {
    logApiPath: '/api/player/hitting-log',
    dateField: 'hittingDate',
    dateLabel: 'Hitting Date',
    savedEntriesLabel: 'Saved BP Template Entries',
    saveButtonLabel: 'Save Hitting',
    emptyMessage: 'No BP templates assigned yet.',
  },
};
...
// 'bubble:<categoryId>' -- see lib/bullpen-column-types.ts.
type BullpenColumnType = 'auto' | 'text' | 'fill' | 'velocity' | 'strike' | 'two-thirds' | string;
const DEFAULT_COLUMN_TYPE: BullpenColumnType = 'auto';
const ALLOWED_COLUMN_TYPES = new Set<string>(['auto', 'text', 'fill', 'velocity', 'strike', 'two-thirds']);
...
function isEditableColumn(col: string, type: BullpenColumnType = DEFAULT_COLUMN_TYPE) {
  ...
  return (
    resolvedType === 'velocity' ||
    resolvedType === 'strike' ||
    resolvedType === 'two-thirds' ||
    resolvedType === 'fill' ||
    isBubbleColumnType(resolvedType)
  );
}
...
function hasTrackingValue(col: string, value: string, type: BullpenColumnType = DEFAULT_COLUMN_TYPE) {
  ...
  if (isBubbleColumnType(resolvedType)) return raw.length > 0;
  return false;
}
...
// Widened to also accept 'bubble:<categoryId>' as a selectable trend metric.
type BullpenTrendMetric = 'velocity' | 'strike' | 'two-thirds' | string;
...
type BullpenSummaryRow = {
  ...
  // categoryId -> option -> count, for whatever bubble categories appear in
  // this group's rows. Percentages are derived at render time (count /
  // sum-of-counts-for-that-category), not stored, so the table can show
  // e.g. "Weak 20% / Solid 55% / Barrel 25%" per category per row.
  bubbleCounts: Record<string, Record<string, number>>;
};
...
const BUBBLE_OPTION_COLORS = ['#60a5fa', '#f97316', '#22c55e', '#a855f7', '#ef4444', '#eab308', '#14b8a6', '#ec4899'];

function BubbleOptionTrendChart({
  dates,
  options,
  byDate,
  totalsByDate,
  ...
  if (!dates.length) {
    return (
      <div style={{ padding: '12px 0', color: '#94a3b8', fontSize: 13 }}>
        No {categoryLabel} trend data yet.
      </div>
    );
  }
  const W = Math.max(720, dates.length * 96 + 92);
  const H = 310;
  const pad = { top: 28, right: 22, bottom: 68, left: 58 };
  const chartW = W - pad.left - pad.right;
  const chartH = H - pad.top - pad.bottom;
  const barWidth = Math.min(48, chartW / Math.max(1, dates.length) - 16);
  const xStep = dates.length > 1 ? chartW / dates.length : chartW;
  const scaleX = (index: number) => pad.left + index * xStep + xStep / 2 - barWidth / 2;
  const scaleY = (percent: number) => pad.top + (1 - percent / 100) * chartH;
  const ticks = [0, 25, 50, 75, 100];
  const colorByOption = new Map(options.map((opt, i) => [opt, BUBBLE_OPTION_COLORS[i % BUBBLE_OPTION_COLORS.length]] as const));
  return (
    <div style={{ overflowX: 'auto', paddingBottom: 4 }}>
      <svg width={W} height={H} role="img" aria-label={`${categoryLabel} by date`} style={{ display: 'block' }}>
        <rect x={0} y={0} width={W} height={H} rx={8} fill="rgba(2,6,23,0.18)" />
        {ticks.map((tick) => (
          <g key={tick}>
            <line x1={pad.left} x2={W - pad.right} y1={scaleY(tick)} y2={scaleY(tick)} stroke="rgba(148,163,184,0.14)" strokeWidth={1} />
          ...
          return (
            <g key={date}>
              {options.map((opt) => {
                ...
                const yTop = scaleY(cumulativePercent + percent);
                const yBottom = scaleY(cumulativePercent);
                ...
                return (
                  <rect
                    ...
                    fill={colorByOption.get(opt) ?? '#60a5fa'}
                    opacity={0.85}
                  >
                    <title>{`${date}\n${opt}: ${count} (${percent.toFixed(1)}%)\nTotal: ${total}`}</title>
                  </rect>
                );
              })}
              <text x={x + barWidth / 2} y={H - 38} textAnchor="middle" fontSize={11} fontWeight={800} fill="#e2e8f0">
                {formatTrendDate(date)}
              </text>
            </g>
          );
        ...
        <text x={pad.left} y={18} fontSize={12} fontWeight={800} fill="#cbd5e1">
          {categoryLabel} by Date
        </text>
      </svg>
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', paddingTop: 6, paddingLeft: pad.left }}>
        {options.map((opt) => (
          <span key={opt} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#cbd5e1' }}>
            <span style={{ width: 10, height: 10, borderRadius: 3, background: colorByOption.get(opt) ?? '#60a5fa', display: 'inline-block' }} />
            {opt}
          </span>
        ))}
      </div>
...
/** Reads the per-entry bubble-category snapshot embedded at save time
 * (__templateBubbleCategories), keyed by category id -> {label, options}.
 * Falls back to an empty map for rows saved before this field existed. */
function getLogEntryBubbleCategories(entry: LogEntry): Record<string, { label: string; options: string[] }> {
  const firstRow = entry.rowsJson.find((row) => row && typeof row === 'object');
  const raw = String(firstRow?.__templateBubbleCategories ?? '').trim();
  if (!raw) return {};
  try {
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      return parsed as Record<string, { label: string; options: string[] }>;
    }
  } catch {
...
export default function ScriptEntry({
  ...
  useEffect(() => {
    ...
    fetch(`${config.logApiPath}?playerId=${playerId}${previewQuery ? `&${previewQuery.slice(1)}` : ''}`, { cache: 'no-store' })
      .then((r) => r.json())
      .then((payload: { entries?: Array<Record<string, unknown>> }) => {
        const raw = Array.isArray(payload.entries) ? payload.entries : [];
        const entries = raw.map((entry): LogEntry => ({
          templateId: String(entry.templateId ?? ''),
          entryDate: String(entry[config.dateField] ?? ''),
          rowsJson: Array.isArray(entry.rowsJson) ? entry.rowsJson as Array<Record<string, string>> : [],
        }));
        setLogEntries(entries);
      })
      .catch(() => {})
      .finally(() => setLoadingEntries(false));
  }, [visibleTemplates, playerId, previewQuery, config.logApiPath, config.dateField]);

  const [bubbleCategories, setBubbleCategories] = useState<BubbleCategoryDef[]>([]);
  useEffect(() => {
    fetch('/api/bubble-categories', { cache: 'no-store' })
      .then((r) => r.json())
      .then((payload: { categories?: BubbleCategoryDef[] }) => {
        setBubbleCategories(Array.isArray(payload.categories) ? payload.categories : []);
      })
      .catch(() => {});
  }, []);
  const bubbleCategoryById = useMemo(() => new Map(bubbleCategories.map((c) => [c.id, c] as const)), [bubbleCategories]);

  // Load existing entry for selected date+template
  useEffect(() => {
    if (!template || !bullpenDate) return;
    const existing = logEntries.find((e) => e.templateId === selectedTemplateId && e.entryDate === bullpenDate);
    if (existing) {
  ...
  const handleSave = async () => {
    ...
    setSaveMsg('');
    try {
      const res = await fetch(config.logApiPath, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          playerId,
          templateId: selectedTemplateId,
          [config.dateField]: bullpenDate,
          rowsJson: rows.map((row) => ({
            ...
            __templateBubbleCategories: JSON.stringify(
              // Snapshot only the categories actually referenced by this
              // template's columns, and only their label/options -- a
              // category's options can be edited later, so this keeps
              // historical rows self-describing as of save time (matching
              // __templateColumns/__templateColumnTypes's existing role for
              // immutable historical data), independent of the live catalog.
              templateColumnTypes.reduce<Record<string, { label: string; options: string[] }>>((acc, type) => {
                const categoryId = bubbleCategoryIdFromType(type);
                if (!categoryId || acc[categoryId]) return acc;
                const category = bubbleCategoryById.get(categoryId);
                if (category) acc[categoryId] = { label: category.label, options: category.options };
                return acc;
              }, {})
            ),
          })),
      ...
      setSaveMsg('Saved!');
      // Refresh entries
      const refresh = await fetch(`${config.logApiPath}?playerId=${playerId}${previewQuery ? `&${previewQuery.slice(1)}` : ''}`, { cache: 'no-store' });
      const refreshPayload: { entries?: Array<Record<string, unknown>> } = await refresh.json();
      const rawEntries = Array.isArray(refreshPayload.entries) ? refreshPayload.entries : [];
      const entries = rawEntries.map((entry): LogEntry => ({
        templateId: String(entry.templateId ?? ''),
        entryDate: String(entry[config.dateField] ?? ''),
        rowsJson: Array.isArray(entry.rowsJson) ? entry.rowsJson as Array<Record<string, string>> : [],
      }));
  ...
  const allRows = useMemo(() => {
    return logEntries
      ...
      .flatMap((entry) => {
        ...
        const bubbleCategoriesSnapshot = getLogEntryBubbleCategories(entry);
        const bubbleCols = columns
          .map((column, index) => ({ column, categoryId: bubbleCategoryIdFromType(columnTypes[index] ?? '') }))
          .filter((entry): entry is { column: string; categoryId: string } => !!entry.categoryId);
        return (entry.rowsJson ?? []).map((row, index): Record<string, string> => ({
          ...row,
          __date: entry.entryDate,
          ...
          __bubbleCols: JSON.stringify(bubbleCols),
          __bubbleCategoriesSnapshot: JSON.stringify(bubbleCategoriesSnapshot),
        }));
      });
  ...
  // Merges the live catalog with each row's own embedded snapshot (so a
  // category later renamed or deleted doesn't blank out historical labels)
  // -- live catalog wins when both exist, since it's more likely current.
  const bubbleCategoryLabelById = useMemo(() => {
    const map = new Map<string, string>();
    for (const row of allRows) {
      const raw = String(row.__bubbleCategoriesSnapshot ?? '').trim();
      if (!raw) continue;
      try {
        const parsed = JSON.parse(raw) as Record<string, { label: string; options: string[] }>;
        for (const [id, def] of Object.entries(parsed)) {
          if (!map.has(id)) map.set(id, def.label);
        }
      } catch {
        // Ignore malformed snapshots.
      }
    }
    for (const category of bubbleCategories) map.set(category.id, category.label);
    return map;
  }, [allRows, bubbleCategories]);
  ...
  const trendMetricOptions = useMemo(() => {
    ...
    const bubbleCategoryIds = Array.from(
      new Set(allTypes.map((type) => bubbleCategoryIdFromType(type)).filter((id): id is string => !!id))
    );
    for (const categoryId of bubbleCategoryIds) {
      const label = bubbleCategoryLabelById.get(categoryId) ?? 'Bubble Category';
      options.push({ value: bubbleColumnType(categoryId), label });
    }
    return options;
  }, [visibleTemplates, logEntries, templateById, bubbleCategoryLabelById]);
  ...
  const combinedTrendData = useMemo(() => {
    if (isBubbleColumnType(trendMetric)) return [];
  ...
  // Distinct from combinedTrendData: a bubble category can have any number
  // of options, so it doesn't reduce to a single "% yes" line the way
  // strike/two-thirds do -- instead this computes, per date, the count and
  // percentage of EACH option so the chart/table can show a full breakdown.
  const bubbleTrendCategoryId = bubbleCategoryIdFromType(trendMetric);
  const bubbleTrendData = useMemo(() => {
    if (!bubbleTrendCategoryId) return { dates: [] as string[], options: [] as string[], byDate: new Map<string, Record<string, number>>(), totalsByDate: new Map<string, number>() };
    const options = new Set<string>();
    for (const category of bubbleCategories) {
      if (category.id === bubbleTrendCategoryId) category.options.forEach((opt) => options.add(opt));
    }
    ...
    for (const row of filteredTrendRows) {
      const bubbleCols = (() => {
        try {
          const parsed = JSON.parse(String(row.__bubbleCols ?? '[]')) as Array<{ column: string; categoryId: string }>;
          return Array.isArray(parsed) ? parsed : [];
        } catch {
          return [];
        }
      })();
      const match = bubbleCols.find((entry) => entry.categoryId === bubbleTrendCategoryId);
      if (!match) continue;
      const date = String(row.__date ?? '').trim();
      if (!date) continue;
      const rawValue = String(row[match.column] ?? '').trim();
      if (!rawValue) continue;
    ...
    return { dates, options: Array.from(options), byDate, totalsByDate };
  }, [filteredTrendRows, bubbleTrendCategoryId, bubbleCategories]);
  ...
  const bullpenSummary = useMemo(() => {
    ...
    const addBubbleCounts = (group: SummaryAccumulator, row: Record<string, string>) => {
      let bubbleCols: Array<{ column: string; categoryId: string }> = [];
      try {
        const parsed = JSON.parse(String(row.__bubbleCols ?? '[]'));
        if (Array.isArray(parsed)) bubbleCols = parsed;
      } catch {
        // Ignore malformed metadata.
      }
      for (const { column, categoryId } of bubbleCols) {
        const value = String(row[column] ?? '').trim();
        if (!value) continue;
        const categoryCounts = group.bubbleCounts[categoryId] ?? {};
        categoryCounts[value] = (categoryCounts[value] ?? 0) + 1;
        group.bubbleCounts[categoryId] = categoryCounts;
      }
    };
    ...
    for (const row of filteredTrendRows) {
      ...
      const hasBubbleValue = String(row.__bubbleCols ?? '[]') !== '[]';
      if (velocity === null && !isStrikeResult(strike) && !isTwoThirdsResult(twoThirds) && !hasBubbleValue) continue;
      ...
      addBubbleCounts(group, row);
      addBubbleCounts(total, row);
      groups.set(key, group);
    }
    ...
    const toSummaryRow = (key: string, group: SummaryAccumulator): BullpenSummaryRow => ({
      ...
      bubbleCounts: group.bubbleCounts,
    });
  ...
  const savedEntrySummaries = useMemo(() => logEntries.map((entry) => {
    ...
    return {
      key: `${entry.templateId}|${entry.entryDate}`,
      templateId: entry.templateId,
      date: entry.entryDate,
      name: sourceTemplate?.name || savedName || (mode === 'hitting' ? 'Saved Hitting' : 'Saved Bullpen'),
  ...
  const summaryBubbleCategoryIds = trendMetricOptions
    .map((option) => bubbleCategoryIdFromType(option.value))
  ...
  if (!visibleTemplates.length) {
    return <p className="portal-muted-text" style={{ margin: 0 }}>{config.emptyMessage}</p>;
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <label style={{ display: 'grid', gap: 4 }}>
          <span style={{ fontSize: 12, color: '#94a3b8' }}>{config.dateLabel}</span>
          <input
            type="date"
            className="portal-schedule-control"
            value={bullpenDate}
        ...
        >
          {saving ? 'Saving...' : config.saveButtonLabel}
        </button>
                  ...
                  return (
                    <tr key={ri} aria-current={isCurrentRow ? 'step' : undefined} style={{ height: 46 }}>
                      ...
                      {template.columns.map((col, ci) => {
                        ...
                        if (isBubbleColumnType(columnType)) {
                          const categoryId = bubbleCategoryIdFromType(columnType);
                          const category = categoryId ? bubbleCategoryById.get(categoryId) : undefined;
                          const options = category?.options ?? [];
                          return (
                            <td key={ci} style={{ ...bullpenCellStyle, ...currentRowCellStyle }}>
                              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', padding: '4px 0' }}>
                                {options.length ? (
                                  options.map((opt) => (
                                    <label key={opt} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, cursor: 'pointer', fontSize: 11, color: isCurrentRow ? '#d1fae5' : '#94a3b8', fontWeight: isCurrentRow ? 800 : 400 }}>
                                      {opt}
                                  ...
                                  <span style={{ fontSize: 11, opacity: 0.6 }}>No options</span>
                                )}
                              </div>
                            </td>
                          );
      ...
      {savedEntrySummaries.length > 0 && !loadingEntries ? (
        <div className="portal-panel" style={{ minHeight: 'unset', padding: '1rem' }}>
          <h4 style={{ margin: '0 0 10px', fontSize: 14, fontWeight: 800, color: '#e2e8f0' }}>{config.savedEntriesLabel}</h4>
      ...
      {template && logEntries.length > 0 && !loadingEntries && trendMetricOptions.length > 0 ? (
        <div className="portal-panel" style={{ minHeight: 'unset', padding: '1rem', display: 'grid', gap: 14 }}>
          ...
          ) : isBubbleColumnType(trendMetric) ? (
            <BubbleOptionTrendChart
              dates={bubbleTrendData.dates}
              options={bubbleTrendData.options}
              byDate={bubbleTrendData.byDate}
              totalsByDate={bubbleTrendData.totalsByDate}
              categoryLabel={trendMetricOptions.find((option) => option.value === trendMetric)?.label ?? 'Bubble Category'}
            />
          ) : (
                ...
                <table className="portal-table portal-bullpen-summary-table">
                  <thead>
                    <tr>
                      ...
                      {summaryBubbleCategoryIds.map((categoryId) => (
                        <th key={categoryId} className="portal-bullpen-summary-number">
                          {bubbleCategoryLabelById.get(categoryId) ?? 'Bubble Category'}
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[...bullpenSummary.rows, bullpenSummary.total].map((summaryRow) => (
                      <tr key={summaryRow.key} className={summaryRow.key === 'all' ? 'portal-bullpen-summary-all' : undefined}>
                        ...
                        {summaryBubbleCategoryIds.map((categoryId) => {
                          const counts = summaryRow.bubbleCounts[categoryId] ?? {};
                          const total = Object.values(counts).reduce((sum, n) => sum + n, 0);
                          const breakdown = Object.entries(counts)
                            .sort((a, b) => b[1] - a[1])
                            .map(([opt, n]) => `${opt} ${total ? Math.round((n / total) * 100) : 0}%`)
                            .join(' / ');
                          return (
                            <td key={categoryId} className="portal-bullpen-summary-number" style={{ fontSize: '0.78rem', whiteSpace: 'nowrap' }}>
                              {breakdown || '—'}
                            </td>
                          );
                        })}
