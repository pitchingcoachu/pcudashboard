...
const numeric=(row:Record<string,unknown>,keys:string[])=>{for(const key of keys){const normalizedKey=key.toLowerCase();const actual=Object.keys(row).find((candidate)=>candidate.toLowerCase()===normalizedKey);const raw=actual?row[actual]:undefined;if(raw===null||raw===undefined||raw==='')continue;const n=Number(raw);if(Number.isFinite(n))return n;}return null;};
const text=(row:Record<string,unknown>,keys:string[])=>{for(const key of keys){const normalizedKey=key.toLowerCase();const actual=Object.keys(row).find((k)=>k.toLowerCase()===normalizedKey);const v=String(actual?row[actual]??'':'').trim();if(v)return v;}return '';};
...
export function evaluateFlagRules(rules:FlagRuleRow[],pointsByDomain:{pitching:Array<Record<string,unknown>>;hitting:Array<Record<string,unknown>>}):FlagResult[]{
  ...
  for(const rule of rules.filter((r)=>r.enabled)){
    ...
    for(const row of rows){
      ...
      const value=numeric(row,keys);if(value===null)continue;
