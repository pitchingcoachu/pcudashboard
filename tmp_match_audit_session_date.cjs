...
(async()=>{
 ...
 const q = await c.query(`
   with pe as (
     select id,
            (case when session_date::text ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' then session_date::date else null end) as d,
     ...
     where coalesce(session_type,'') ilike '%live%'
       and (case when session_date::text ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' then session_date::date else null end) <= '2026-02-13'::date
